function f = FastDRAL_tvt_single(dataset,nSelInsArr,alpha_candi,beta_candi)
%%
...
%% setup
maxIter = 20;
[fea,gnd,fea_can,gnd_can,fea_val,gnd_val,fea_te,gnd_te] = loaddata_tvt(dataset);
[nSmp,nFea] = size(fea);
...
for iSelIns = 1:length(nSelInsArr)
    ...
    for para_idx = 1:length(paras)
        ...
        t_start = clock;
        SelIdx = FastDRAL(fea_can', k, alpha, beta, maxIter, options);
        t_end = clock;
    ...
    for para_idx = 1:length(paras)
        ...
        labeledIdx = InsIdx;
        trFea = fea_can(labeledIdx,:);
        trGnd = gnd_can(labeledIdx);
        valFea = fea_val;
        valGnd = gnd_val;
        teFea = fea_te;
        teGnd = gnd_te;
    ...
    %     disp(['unique_trGnd_cnt:',num2str(unique_trGnd_cnt)]);
        performance = svm_ova_tvt_mm(trFea, trGnd, valFea, valGnd, teFea, teGnd);
        performance_knn = KNN_Classifier_tvt_mm(trFea, trGnd, valFea, valGnd, teFea, teGnd);
...
Method_name = 'FastDRAL_tvt_kmInit'; %kmInit, mvInit, kmInit_embedding
